...
class SimpleDenseAgent(tf.keras.layers.Layer):
    ...
    def __init__(self, nInst : int, config : Config, name : str = None, dtype : tf.DType = dh_dtype ):
        ...
        self.gru_state = config("use_gru", False, bool, "Whether or not to use GRU")
        self.gru_units = config("gru_units", 32, Int>=0, "number of GRU units" )

        if self.gru_state:
            self.classic_states     = 0
            self.aggregate_states   = 0
            self.past_repr_states   = 0
            self.event_states       = 0
        else:
        ...
        if not self.gru_state:
            ...
            self.bound_aggr_states  = False
            self.sigmoid_1          = False
       
        self.nInst              = int(nInst)
        self.nUpdateUnits       = self.aggregate_states + self.past_repr_states + self.event_states 
        self.nStates            = self.gru_units if self.gru_state else (self.classic_states + self.aggregate_states + self.past_repr_states + self.event_states) 
        ...
        self._layer             = DenseLayer( features=features, nOutput=nOutput, config=config.network, name=name+"_layer", dtype=dtype, use_gru=self.gru_state, gru_units=self.gru_units )
    ...
    def call( self, all_features : dict, training : bool = False ) -> tuple:
        ...
        if self.gru_state:
            ...
            out_state = output[:, self.nInst:self.nInst + self.nStates]
...
def AgentFactory( nInst : int, config : Config, name : str = None, dtype=dh_dtype ) -> tf.keras.layers.Layer:
    ...
    # Check if it's a dict-like object and use get()
    if hasattr(config, "get"):
        agent_type = config.get("agent_type", "feed_forward")
    else:
        # Fallback for the Configer object
